...
class State(object):
	...
	def initialize(self):
		...
		for u in self.g:
			...
			self.adj_list[u] = list(self.g.neighbors(u))[:]
	...
	def neighbors(self, i):
		if type(i) == list:
			nbrs = []
			for idx in i:
				nbrs += list(self.partition_graph.neighbors(idx))  # Convert to list
			nbrs = set(nbrs)
			nbrs.difference_update(i)
			return nbrs
		else:
			assert type(i) == int or type(i) == str
			nbrs = list(self.partition_graph.neighbors(i))  # Convert to list
			if i in nbrs:
				nbrs.remove(i)
			return nbrs
